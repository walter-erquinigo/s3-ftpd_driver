...
class DevFTPDriver
	...
	def change_dir(path, &block)
		yield (path == '/' or
				@bucket.objects[without_root_slash(path)].exists?)
	end
	...
	private
	  ...
	  # 	
		def is_dir?(dir, level)
			...
			dir[-1] == '/' && 
			(level == :root_path_level && dir.count('/') == 1 ||
					level == :any_dir_level)
		end
		...
		def is_file?(file, level)
			!is_dir?(file, level) &&
					!dir.empty? &&
					(level == :root_path_level &&
					without_root_slash(file).count('/') == 0 ||
			 		level == :any_dir_level)
		end
